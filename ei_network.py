...
class ei_network():
    ...
    def __init__(self, layers, net_number=None):
        ...
        if type(layers) is list and type(layers[0]) is str:
            for name in layers:
                setattr(self, name, ei_layer(name, self.net_params))
        else:
    ...
    def run(self, n_pres,  pres_time, set_inp_pat=None, set_inp_fun=None):
        """ Run a simulation, presenting n_pres patterns, each lasting pres_time. 
            ...
            Args:
                ...
                set_inp_pat : a dictionary with the methods that set the input patterns to be presented
                              ...
                              This creates a new pattern to be presented. The set_inp_pat method has
        ...
        for name in self.layers:
            lay = self.__getattribute__(name)
            if lay.n['x'] > 0:  # If the layer has external input units
                    inp_pat[name] = np.zeros(lay.n['x'])
